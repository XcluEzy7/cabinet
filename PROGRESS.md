...
[2026-04-29] Added production `Dockerfile` for Cabinet enabling containerized deployment with multi-stage build (builder + runner). Builder stage installs native build tools (python3 make g++) for `node-pty`. Runner includes git, curl, built app, and runs `npm run start` on port 3000 + daemon on 3001. Deployed as `hermes-workspace:latest` image via `~/runtipex/hermes-workspace/docker-compose.yml` and integrated into `~/runtipex/docker-compose.yml` with Traefik labels for `wally.tech4x.org`, Authelia middleware, Let's Encrypt, and `traefik_proxy` network. Workspace healthcheck returns `{ "status":"ok", "version":"0.3.4" }` on port 3000.
